...
function NextFixtureCard() {
  ...
  return (
    <motion.div
      ...
      style={{ marginTop: 16, borderRadius: 22, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.18)', boxShadow: '0 26px 64px -20px rgba(37,40,120,0.62), 0 0 40px -16px rgba(124,58,237,0.5), inset 0 1px 0 rgba(255,255,255,0.26)', position: 'relative', backdropFilter: 'blur(20px) saturate(160%)', WebkitBackdropFilter: 'blur(20px) saturate(160%)', background: 'radial-gradient(130% 90% at 12% -10%, rgba(34,211,238,0.10), transparent 55%), linear-gradient(160deg, rgba(24,27,38,0.92), rgba(13,15,22,0.94))' }}
    >
      {/* Decorative blobs */}
    ...
    <motion.div variants={staggerItem} style={{ marginTop: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        {stats.map(({ label, value }) => {
          ...
          const VALUE  = '#a5f3fc'      // bright cyan number
          return (
          <div key={label} style={{
            background: 'radial-gradient(130% 90% at 12% -10%, rgba(34,211,238,0.10), transparent 55%), linear-gradient(160deg, rgba(24,27,38,0.92), rgba(13,15,22,0.94))',
            borderRadius: 18,
            ...
            boxShadow: `0 8px 22px rgba(0,0,0,0.4), inset 0 0 28px ${ACCENT}12`,
            ...
            <div style={{ position: 'absolute', bottom: -14, right: -14, width: 48, height: 48, borderRadius: '50%', background: `${ACCENT}22`, pointerEvents: 'none', filter: 'blur(7px)' }} />
            <div style={{ fontFamily: FONT, fontSize: 28, fontWeight: 900, color: VALUE, lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>{value}</div>
            <div style={{ fontFamily: FONT, fontSize: 9, fontWeight: 800, color: 'rgba(165,180,252,0.6)', marginTop: 7, textTransform: 'uppercase', letterSpacing: 0.8 }}>{label}</div>
          </div>
          )
        })}
      </div>
                ...
                <motion.div
                  ...
                  style={{
                    background: m.is_active
                      ? 'linear-gradient(160deg, rgba(88,40,180,0.42), rgba(30,18,70,0.72))'
                      : 'linear-gradient(160deg, rgba(20,24,48,0.78), rgba(11,14,30,0.82))',
                    backdropFilter: 'blur(14px) saturate(150%)',
                    WebkitBackdropFilter: 'blur(14px) saturate(150%)',
                    borderRadius: 16,
                    padding: '14px 16px',
                    boxShadow: m.is_active ? '0 10px 30px -12px rgba(124,58,237,0.5), inset 0 1px 0 rgba(255,255,255,0.12)' : '0 6px 18px rgba(0,0,0,0.32), inset 0 1px 0 rgba(255,255,255,0.06)',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 14,
                    border: m.is_active ? `1.5px solid ${C.green}` : '1px solid rgba(255,255,255,0.09)',
                  }}
                >
                  <div
                    style={{
